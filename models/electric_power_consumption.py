"""
Provides electric power model that can be fitted and inferred.
...
class ElectricPowerModel(Model):
    """
    States the Electric Power Model that will be used to calculate natural gas
    consumption.
    ...
    def _calculate_estimated_eia_monthly_data(self, idata):
        ...
        eia_observations_by_month["Date"] = pd.to_datetime(
            eia_observations_by_month.apply(lambda row: datetime.datetime(int(row["Year"]), int(row["Month"]), 1),
                                            axis=1))
    ...
    def inference(self,
        ...
        with pm.Model(coords=coords) as model:
            ...
            eia_daily_observations = pm.Normal("eia_observations",
                                               mu=(
                                                              alpha + alpha_2) * consumption_factor + alpha_2 * consumption_factor_lagged + minimum_consumption,
                                               sigma=params.get("daily_consumption_error"),
                                               dims="dates")

            consumption_factor_to_index, index_to_consumption_factor_date, dates = map_date_to_index(
                data["consumption_factor_values"])
            ...
            eia_estimated_daily_observations, estimated_estimated_monthly_data = self._calculate_estimated_eia_monthly_data(
                idata)
...
def load_electric_power_data(state,
    ...
    if consumption_factor_method == "POPULATION_WEIGHTED_CDD":
        ...
        consumption_factor = calculate_consumption_factor_via_pop_weighted_weather(population_weighted_weather,
                                                                                   ...
                                                                                   state,
                                                                                   degree_day_type="CDD")
    elif consumption_factor_method == "POPULATION_WEIGHTED_HDD":
        raise NotImplementedError("Electric Power Consumption Model does not rely on HDD. "
                                  "Electric Power Consumption Model relies on CDD. "
                                  "Please try consumption_factor_method=='POPULATION_WEIGHTED_CDD")
...
def fit_electric_power_model(start_training_time: str,
    ...
    if method == "GLOBAL":
        best_parameters, optimal_rel_error = ElectricPowerModel(calibrated_parameters,
